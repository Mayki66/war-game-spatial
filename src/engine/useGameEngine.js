...
import { getManeuverCost } from '../rules/constants';
...
export function useGameEngine() {
  ...
  const [scores, setScores] = useState({ celtica: 0, mercure: 0 });
  const [intelUntil, setIntelUntil] = useState({ celtica: 0, mercure: 0 }); 
  const [logs, setLogs] = useState([{ id: 0, text: "[SYSTEME] Moteur strategique operationnel.", color: "#9ca3af" }]);
    ...
    if (roomId && gameMode === 'online') {
      ...
      const unsubscribe = onValue(roomRef, (snapshot) => {
        ...
        if (data) {
          ...
          if(data.sats) setSats(data.sats); if(data.scores) setScores(data.scores); if(data.turn) setTurn(data.turn);
          if(data.currentPlayer) setCurrentPlayer(data.currentPlayer); if(data.intelUntil) setIntelUntil(data.intelUntil);
          if(data.logs) setLogs(data.logs); if(data.winner) setWinner(data.winner);
          setGameStarted(true);
        }
      });
      return () => unsubscribe();
  ...
  const startGame = (mode = 'hotseat', faction) => {
    ...
    if (mode === 'online') {
      ...
      const initialState = { hostFaction: faction, sats: initialSatellites, scores: { celtica: 0, mercure: 0 }, turn: 1, currentPlayer: 'Mercure', winner: null, intelUntil: { celtica: 0, mercure: 0 }, logs: [{ id: Date.now(), text: `[C2] Salon ${newRoom} cree.`, color: "#38bdf8" }] };
  ...
  // Verifie le cout en ergol en prenant en compte l'orbite cible
  ...
  const toggleOperational = (satId) => {
    ...
    syncState({ sats: newSats, logs: getLogArray(`[C2] ${newSats[satId].name} passe en ${newState}.`, newSats[satId].color) });
  };
  ...
  const launchSatellite = (satId, altitude, inclination) => {
    ...
    syncState({ sats: newSats, logs: getLogArray(`[ORDRE] Lancement programme (${delay} tours).`, newSats[satId].color) });
  };
  ...
  const startRPO = (attackerId, targetId, customTurns) => {
    ...
    syncState({ sats: newSats, logs: getLogArray(`[C2] RPO ordonnee sur ${target.name}.`, attacker.color) });
  };
  ...
  const startCollision = (attackerId, targetId, customTurns) => {
    ...
    syncState({ sats: newSats, logs: getLogArray(`[C2] Protocole de destruction engage sur ${target.name}.`, attacker.color) });
  };
  ...
  const startEvasion = (satId, newAltitude) => {
    ...
    syncState({ sats: newSats, logs: getLogArray(`[C2] Evasion d'urgence initiee.`, newSats[satId].color) });
  };
  ...
  const requestAllyIntel = () => {
    ...
    if(allySat) {
      ...
      syncState({ sats: newSats, logs: getLogArray(`[C2] Requete d'intel envoyee aux Allies.`, "#22c55e") });
    }
  };
  ...
  const processTurnResolution = (currentSats, currentScores, currentIntel, activePlayer) => {
    ...
    let nextScores = { ...currentScores }; let nextIntel = { ...currentIntel }; let logsToAdd = [];
    ...
    Object.values(nextSats).forEach(sat => {
      if ((sat.owner === activePlayer || (activePlayer === 'Allié' && sat.owner === 'Allié')) && sat.task) {
        ...
        if (sat.task.turnsLeft <= 0) {
          const task = sat.task;
          if (task.type === 'LAUNCH') { sat.isActive = true; sat.radius = task.targetRadius; sat.currentRadius = task.targetRadius; sat.inclination = task.targetInclination * (Math.PI / 180); sat.status = "Opérationnel"; logsToAdd.push({ text: `[DEPLOIEMENT] ${sat.name} est en orbite !`, color: sat.color }); }
            ...
            if (sat.jammedBy) { if (nextSats[sat.jammedBy]) { nextSats[sat.jammedBy].isRPO = false; nextSats[sat.jammedBy].targetId = null; nextSats[sat.jammedBy].status = "Opérationnel"; } sat.jammedBy = null; nextScores[sat.owner.toLowerCase()] += 10; logsToAdd.push({ text: `[DEFENSE] ${sat.name} a brise le verrouillage (+10 pts).`, color: sat.color }); }
          }
            ...
            if (target && target.radius === task.expectedRadius && target.isActive && target.status !== "Non opérationnel") {
              ...
              if (sat.mainMission === 'Renseignement') {
                ...
                if (target.mainMission === 'Scientifique') { nextScores[sat.owner.toLowerCase()] += 5; logsToAdd.push({ text: `[PIRATAGE] ${sat.name} siphone ${target.name} (+5 pts).`, color: sat.color }); }
            ...
            } else { sat.status = "Opérationnel"; logsToAdd.push({ text: `[ECHEC] RPO ratee, cible invalide.`, color: "#9ca3af" }); }
          }
          else if (task.type === 'COLLISION') {
            const target = nextSats[task.targetId];
            if (target && target.radius === task.expectedRadius && target.isActive) {
              sat.isActive = false; target.isActive = false; sat.status = "DÉTRUIT"; target.status = "DÉTRUIT"; nextScores[sat.owner.toLowerCase()] += 10;
              logsToAdd.push({ text: `[IMPACT] ${sat.name} a detruit sa cible (+10 pts).`, color: "#ef4444" });
            } else { sat.status = "Opérationnel"; }
          }
    ...
    Object.values(nextSats).forEach(sat => {
      if (sat.owner === activePlayer && sat.isActive && sat.status === "Opérationnel" && !sat.task && !sat.jammedBy && sat.mainMission === "Scientifique") nextScores[activePlayer.toLowerCase()] += 5;
    });
  ...
  const resolveTurnLoop = (playerToResolve) => {
    ...
    if (nextScores.mercure >= 100) nextWinner = "MERCURE"; else if (nextScores.celtica >= 100) nextWinner = "CELTICA";

    syncState({ sats: nextSats, scores: nextScores, intelUntil: nextIntel, logs: compiledLogs, currentPlayer: nextPlayer, turn: nextTurn, winner: nextWinner });
  };
  ...
  useEffect(() => {
    if (gameStarted && !winner && ((gameMode === 'solo' && currentPlayer !== humanFaction && currentPlayer !== 'Allié') || currentPlayer === 'Allié')) {
      const aiTimer = setTimeout(() => {
        ...
        let nextWinner = nextScores[aiFaction.toLowerCase()] >= 100 ? aiFaction.toUpperCase() : winner;
        syncState({ sats: nextSats, scores: nextScores, intelUntil: nextIntel, logs: compiledLogs, currentPlayer: nextPlayer, winner: nextWinner });
      }, 1500); 
      return () => clearTimeout(aiTimer);
    }
  }, [currentPlayer, gameStarted, gameMode, winner, humanFaction]);

  return { roomId, gameMode, humanFaction, gameStarted, winner, sats, turn, currentPlayer, scores, intelUntil, logs, getMinTurns: getManeuverCost, startGame, endTurn, toggleOperational, launchSatellite, startRPO, startCollision, startEvasion, requestAllyIntel };
}